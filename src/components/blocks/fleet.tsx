...
export async function FleetGridBlock({
  ...
  const [coaches, classes, totalPublished, pricedCount] = await Promise.all([
    prisma.coach
      .findMany({
    ...
    <Section base={props} labelledBy={headingId}>
      ...
      {props.showFilters ? (
        <form method="get" className="mb-8 rounded-card border border-line bg-surface-alt p-5" aria-label="Filter the fleet">
          <div className={cn('grid gap-4 sm:grid-cols-2', showPriceFilter ? 'lg:grid-cols-5' : 'lg:grid-cols-4')}>
            <div>
            ...
            <div>
              ...
              <select id="filter-slides" name="slides" defaultValue={filters.slides} className="kc-field">
                ...
                <option value="Double">Double slide</option>
                <option value="Triple">Triple slide</option>
              </select>
            </div>
            {showPriceFilter ? (
              <div>
              ...
              <Card key={coach.id} as="li" className="overflow-hidden">
                <article>
                  ...
                  <div className="p-6">
                    ...
                    <SmartLink href={href} className="kc-btn kc-btn-primary mt-6 w-full">
                      View {coach.name}
                    </SmartLink>
                  </div>
                </article>
              ...
              {classes.flatMap((cls) =>
                cls.coaches.map((coach) => (
                  <tr key={coach.slug}>
                    ...
                    <td className="border border-line px-4 py-3">{coach.slideOuts}</td>
                    <td className="border border-line px-4 py-3">{coach.rearConfig}</td>
